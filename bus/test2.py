...
idx = pd.MultiIndex.from_product([df['date'].unique(), df['start_time'].unique(), range(segments_down, segments_up + 1)],
...
for i in range(segments_down, segments_up + 1):
    pivoted_df[i] = (pivoted_df[i] - min_values[i-segments_down]) / (max_values[i-segments_down] - min_values[i-segments_down])
